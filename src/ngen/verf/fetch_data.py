...
def retrieve_usgs_obs(conf: dict, locations: list, output_dir:Path):
    ...
    locations: list of USGS gage IDs
...
def retrieve_nwm_fcsts(conf: dict, locations: list, output_dir:dict, json_dir:dict, data_link_dir:dict):

    """
    Retrieve NMW forecasts given the configrations and list of locations

    conf: dictionary defining the configurations (e.g., config.yaml)
    locations: list of NWM feature_id
    ...
    for dataset in conf1['dataset_name']:
        ...
        name = conf1['dataset_name'][i1]
        version = conf1['nwm_version'][i1]
        start_date = conf1['forecast_start_date'][i1]
        end_date = conf1['forecast_end_date'][i1]
        logger.info(f'  ======== Fetch data for NWM dataset {name}: {version} {start_date} to {end_date} ==========')
        ...
        # cycles without existing parquet files 
            ...
            for d1 in dates1:
                ...
                with LocalCluster(n_workers=n_workers,
                    ...
                    tlp.nwm_to_parquet(
                        ...
                        nwm_version = conf1['nwm_version'][i1],
                ...
                logger.info(f'  NWM forecast data are saved in parquet files at: {output_dir}')
