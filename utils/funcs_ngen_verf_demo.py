...
def display_images(
    ...
    if plot_type == "map":
        filenames = [
            f"map_{p1}_{p2}_{p3}.png"
            for p1, p2, p3 in itertools.product(metrics, lead_times, datasets)
        ]
    elif plot_type == "histogram":
        filenames = [
            f"hist_{p1}_{p2}.png" for p1, p2 in itertools.product(metrics, lead_times)
        ]
    ...
    if plot_type == "map":
        ...
        html = "".join(
            f'<img src="{img}?t={time.time()}" style="margin-right: 10px;">'
            for img in images
        )
        display(HTML(html))
